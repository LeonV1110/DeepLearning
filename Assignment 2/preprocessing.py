...
def preprocess_file(filename_path: Path, drop_out_factor = 10):
    # Loading in the data
    ...
    scaled_and_transposed = pd.DataFrame(StandardScaler().fit_transform(dropped_out.T), 
                                        columns=dropped_out.T.columns,
                                        index = dropped_out.T.index)#TODO make scaler fit on all data, not just the current file
...
def main():
    ...
    for folder in (INPUT / Path(DATATYPE)).iterdir():
        if folder == INPUT / Path(DATATYPE) / Path('.DS_Store'):
            print(folder)
            continue
